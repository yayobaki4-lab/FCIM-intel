...
const QUERY_BUCKETS = [
  {
    ...
    body: {
      searchQuery: 'Armenian Azerbaijani Georgian family office wealth',
  ...
  },
  {
    ...
    body: {
      searchQuery: 'Russian CIS Kazakh family office wealth principal',
      currentJobTitles: ['Family Office', 'Principal', 'Investor', 'Chairman'],
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
  ...
  },
  {
    ...
    body: {
      searchQuery: 'Nigerian Kenyan South African Ghanaian family office HNW',
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
  ...
  {
    label: 'MENA family offices Dubai (Levant/N.Africa)',
    body: {
      searchQuery: 'Lebanese Egyptian Moroccan Tunisian Jordanian family office wealth',
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
  ...
  {
    label: 'Commodity traders & physical firms Dubai',
    body: {
      searchQuery: 'physical commodity trading freight fertilizer grain energy hedging',
      currentJobTitles: ['Commodity Trader', 'Trading Director', 'Head of Trading', 'Risk Manager'],
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
  ...
  {
    label: 'Serial founders / investment holdings Dubai',
    body: {
      searchQuery: 'serial entrepreneur investment holding portfolio companies',
      currentJobTitles: ['Founder', 'Chairman', 'Managing Director'],
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
  ...
  {
    label: 'EAMs & boutique wealth managers DIFC/ADGM',
    body: {
      searchQuery: 'external asset manager independent wealth boutique advisory',
      currentJobTitles: ['Managing Partner', 'Founding Partner', 'Wealth Advisor', 'Independent Financial Advisor'],
      locations: ['United Arab Emirates'],
      profileScraperMode: 'Short',
      takePages: 1,
      maxItems: PROFILES_PER_QUERY
...
function prospectCardHtml(p, isFeatured) {
  ...
  const problemBlock = dx.problem
    ? `<div class="section" style="background:#F6EFD8; padding:12px 14px; border-left:3px solid #C9A544; border-radius:4px;">
        <div class="label">Diagnosed problem ${conf}</div>
        <p style="margin-top:6px;"><strong>${escapeHtml(dx.problem)}</strong></p>
        <p style="margin-top:8px; font-size:13px; color:#3C3730;">${escapeHtml(dx.angle || '')}</p>
      </div>`
    : '';
  ...
  return `
    <article class="prospect ${isFeatured ? 'featured' : ''}">
      ...
      <div class="section">
        ...
        <p>${escapeHtml(p.regionWarmPath || 'Region indeterminate. Route to the FCIM colleague best matched to the subject\u2019s nationality / sector.')}</p>
      </div>
      ${complianceBlock}
      <div class="first-step">
        <div class="label">First step</div>
        <p>Approach ${escapeHtml(p.name)} via ${p.regionLead ? escapeHtml(p.regionLead) + '\u2019s network' : 'the appropriate FCIM colleague'}. Tap below to copy the diagnostic-agent draft prompt — paste it in your Claude chat and Yehya\u2019s email comes back, anchored on the diagnosed FCIM angle.</p>
...
function renderServiceSection(svc, items) {
  ...
  const blocks = regionOrder.map(rname => {
    ...
    return `
      <div class="region-block" style="${REGION_BLOCK}">
        <div style="${REGION_ROW}">
          <h3 style="${REGION_NAME}">${escapeHtml(name)}</h3>
          <div style="${REGION_META}">${escapeHtml(lead)} \u00b7 ${ps.length} prospect${ps.length === 1 ? '' : 's'}</div>
        </div>
      </div>
...
function renderRegionChips(regionCounts) {
  return REGIONS.map(r => {
    ...
    return `
      <button class="region-chip ${empty ? 'empty' : ''}" ${empty ? 'disabled' : `data-region="${escapeHtml(r.name)}"`}>
